...
class BaseDecodeHead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        # self.conv_seg_vis = ConvBnLeakyRelu2d(48, 48, kernel_size=1, padding=0)
        # self.conv_seg_ir = ConvBnLeakyRelu2d(48, 48, kernel_size=1, padding=0)
    ...
    def fuse(self, seg_vis, seg_ir, x1, x2):
        # feat = torch.cat([seg_fea, img_fea], dim=1)
        # """Pixel fuse."""
        # if self.dropout is not None:
        #     seg_vis = self.dropout(seg_vis)
        #     seg_ir = self.dropout(seg_ir)
        # img_fea = self.conv_low(img_fea)
        w_vis = seg_vis.repeat(1, 48, 1, 1)
        w_ir = seg_ir.repeat(1, 48, 1, 1)
        # w_vis = self.conv_seg_vis(seg_vis)
        # w_ir = self.conv_seg_ir(seg_ir)
        x1 = x1 + w_vis*x1
        x2 = x2 + w_ir*x2
        img_fea = torch.cat([x1, x2], dim=1)

        out = self.conv_fuse1(img_fea)
