...
class FuserSegmenter(DualBaseSegmenter):
    ...
    def __init__(self,
                 ...
                 no_fusionloss=False,
                 init_cfg=None):
        super(FuserSegmenter, self).__init__(init_cfg)
        self.no_fusionloss = no_fusionloss
        self.fuser = builder.build_fuser(fuser)
    ...
    def encode_decode(self, img_rgb, img_ir, img_metas):
        ...
        img = self.fuser.forward_test(img_rgb, img_ir, img_metas)
        # img = img.unsqueeze(0)
        # print(img[0].shape)
        # exit()
    ...
    def forward_train(self, img_rgb, img_ir, img_metas, gt_semantic_seg):
        ...
        # print(img_rgb)
        # exit()
        img, fusion_loss = self.fuser.forward_train(img_rgb, img_ir)
        x = self.extract_feat(img)

        losses = dict()
        if not self.no_fusionloss:
