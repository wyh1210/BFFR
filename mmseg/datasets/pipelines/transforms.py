...
class DualResize(object):
    ...
    def _resize_img(self, results):
        ...
        if self.keep_ratio:
            ...
            img2 = mmcv.imrescale(
                results['img2'], results['scale'], return_scale=False)
            # the w_scale and h_scale has minor difference
            # a real fix should be done in the mmcv.imrescale in the future
    ...
    def _resize_bin(self, results):
        """Resize binary map with ``results['scale']``."""
        
        if self.keep_ratio:
            bin = mmcv.imrescale(
                results['binary_img'], results['scale'], interpolation='nearest')
        else:
            bin = mmcv.imresize(
                results['binary_img'], results['scale'], interpolation='nearest')
        results['binary_img'] = bin
    ...
    def __call__(self, results):
        ...
        self._resize_img(results)
        self._resize_seg(results)
        if 'binary_img' in results:
            self._resize_bin(results)
        # print(results)
        # print(results['img1'].shape)
        # print(results['img2'].shape)
        # print(results['gt_semantic_seg'].shape)
        # print(results['binary_img'].shape)
        # exit()
...
class DualRandomFlip(object):
    ...
    @deprecated_api_warning({'flip_ratio': 'prob'}, cls_name='DualRandomFlip')
    def __init__(self, prob=None, direction='horizontal'):
        self.prob = prob
        self.direction = direction
        if prob is not None:
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            # flip binary image
            if 'binary_img' in results:
                results['binary_img'] = mmcv.imflip(results['binary_img'], direction=results['flip_direction']).copy()
        # print(results)
        # print(results['binary_img'].shape)
        # print(crop_bbox)
        # exit()
...
class DualPad(object):
    ...
    def _pad_bin(self, results):
        """Pad masks according to ``results['pad_shape']``."""
        results['binary_img'] = mmcv.impad(
            results['binary_img'],
            shape=results['pad_shape'][:2],
            pad_val=self.bin_pad_val)
    ...
    def __call__(self, results):
        ...
        self._pad_seg(results)
        if 'binary_img' in results:
            self._pad_bin(results)

        # print(results)
        # import PIL.Image as Image
        # print(type(results['img1']))
        # print(results['img1'].shape)
        # bin_img = Image.fromarray(results['binary_img'])
        # img = Image.fromarray(np.uint8(results['img1']*255))

        # bin_img.save('bin_img.png')
        # img.save('img1.png')
        # # Image. results['binary_img'].
        # exit()
...
class DualNormalize(object):
    ...
    def __init__(self, mean1, mean2, std1, std2, to_rgb=True):
    ...
    def __call__(self, results):
        ...
        # results['img1'] = mmcv.imnormalize(results['img1'], self.mean1, self.std1,
        #                                   self.to_rgb)
        # results['img2'] = mmcv.imnormalize(results['img2'], self.mean2, self.std2,
        #                                   self.to_rgb)
        results['img1'] = np.float32(results['img1']/255.0)
        results['img2'] = np.float32(results['img2']/255.0)
        results['img_norm_cfg'] = dict(
            mean1=self.mean1, mean2=self.mean2, std1=self.std1, std2=self.std2, to_rgb=self.to_rgb)
        return results
...
class DualRandomCrop(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('seg_fields', []):
            results[key] = self.crop(results[key], crop_bbox)
        if 'binary_img' in results:
            results['binary_img'] = self.crop(results['binary_img'], crop_bbox)
...
@PIPELINES.register_module()
class DualPhotoMetricDistortion(object):
    """Apply photometric distortion to image sequentially, every transformation
    is applied with a probability of 0.5. The position of random contrast is in
    second or second to last.
    ...
    def __call__(self, results):
        ...
        img2 = results['img2']
        # random brightness
        img1 = self.brightness(img1)
        img2 = self.brightness(img2)
        ...
        # random saturation
        img1 = self.saturation(img1)
        img2 = self.saturation(img2)
